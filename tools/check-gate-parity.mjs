...
/**
 * There is a THIRD hand-written copy of this list, and it is the one strangers read: the entry
 * page prints the chain under the words "verbatim from package.json check:full". It had drifted by
 * five entries -- the page showed fourteen steps for a chain that ran nineteen -- because nothing
 * connected the array to the script it claims to quote. A published number with no gate behind it
 * is exactly what check:published-numbers exists to prevent, and this one had slipped past it by
 * being a list rather than a number.
 */
const SHOWCASE = 'src/app/feature/survey/showcases/testing-quality-showcase.component.ts';
const showcaseSource = readFileSync(SHOWCASE, 'utf8');
const arrayLiteral = /readonly feGates = \[([\s\S]*?)\];/.exec(showcaseSource);
const published = arrayLiteral
  ? [...arrayLiteral[1].matchAll(/'([^']+)'/g)].map((m) => m[1])
  : null;

// check:full is check:static followed by the steps that need a compiler or a test runner. Those
// four are named the way the page names them, since `npm test -- --bail` is jest.
const tailSteps = ['typecheck', 'typecheck:e2e', 'build:check', 'jest --bail'];
const expectedPublished = [...staticChecks, ...tailSteps];

let publishedProblem = null;
if (!published) {
  publishedProblem = `could not find the feGates array in ${SHOWCASE}`;
} else if (published.join('|') !== expectedPublished.join('|')) {
  const onlyPublished = published.filter((s) => !expectedPublished.includes(s));
  const onlyReal = expectedPublished.filter((s) => !published.includes(s));
  publishedProblem =
    `the entry page's published chain (${published.length} steps) does not match check:full ` +
    `(${expectedPublished.length} steps)` +
    (onlyReal.length ? `\n  in check:full but NOT published: ${onlyReal.join(', ')}` : '') +
    (onlyPublished.length ? `\n  published but NOT in check:full: ${onlyPublished.join(', ')}` : '') +
    (!onlyReal.length && !onlyPublished.length ? '\n  same members, different order' : '');
}

if (missingFromHook.length === 0 && missingFromStatic.length === 0 && !publishedProblem) {
  console.log(
    `check:gate-parity OK — the hook and check:static run the same ${staticChecks.size} checks, ` +
      `and the entry page publishes the same ${expectedPublished.length}-step check:full chain.`,
  );
  process.exit(0);
}
...
if (publishedProblem) {
  console.error(`check:gate-parity FAILED — ${publishedProblem}`);
  console.error(
    `  ${SHOWCASE} prints this list to visitors as "verbatim from package.json check:full".`,
  );
}
process.exit(1);
