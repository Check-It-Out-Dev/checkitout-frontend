...
(() => {
  ...
  // An allow-list, not a deny-list. Enumerating what is forbidden means being right about every
  // way to write an absolute URL; enumerating what is permitted means being right once. A segment
  // of unreserved characters cannot carry a scheme (no `:`), cannot start a protocol-relative
  // reference or an absolute path (both need an empty first segment, and `+` requires one
  // character), and `..` is refused by name.
  ...
  const BASE = (() => {
    ...
    if (!safe) {
      console.warn(
        'dashboard: ignoring ?data=%s — only a relative path on this origin is read',
        raw,
      );
      return '.';
    }
  ...
  function el(tag, attrs = {}, ...children) {
    ...
    for (const c of children.flat())
      if (c != null) e.append(c.nodeType ? c : document.createTextNode(String(c)));
    return e;
  }
  ...
  function table(headers, rows, opts = {}) {
    const thead = el(
      ...
      el(
        'tr',
        {},
        headers.map((h) => el('th', { class: h.num ? 'num' : null, scope: 'col', text: h.text })),
      ),
    );
    const tbody = el(
      'tbody',
      ...
      rows.map((r) =>
        el(
          'tr',
          { class: r.total ? 'total' : null },
          r.cells.map((c) => el('td', { class: c.num ? 'num' : null }, c.node ?? c.text)),
        ),
      ),
    );
    return el(
      'div',
      { class: 'tablewrap' },
      el('table', { 'aria-label': opts.label }, thead, tbody),
    );
  }
  ...
  function renderVerdict(m, history) {
    ...
    if (r.status === 'incomplete')
      text = `Run ${r.number} on ${r.branch} did not finish: ${n(t.passed)} of ${n(scored)} tests reported, the rest never ran.`;
    else if (t.failed > 0)
      text = `${n(t.failed)} of ${n(scored)} tests failed on ${r.branch}, run ${r.number}.`;
    else if (t.flaky > 0)
      text = `${n(t.passed)} of ${n(scored)} tests passed on ${r.branch}, run ${r.number}: ${word(t.flaky)} flaky, none failed.`;
    ...
    const runs = [
      { run: r.number, failed: t.failed, status: r.status },
      ...history.filter((h) => h.run !== r.number).reverse(),
    ];
    if (r.status === 'incomplete') {
      parts.push('An unfinished run breaks the streak: it is neither green nor red.');
    } else if (t.failed > 0) {
      let green = 0;
      for (const h of runs.slice(1)) {
        if (!clean(h)) break;
        green++;
      }
      if (green)
        parts.push(`The first red run after ${word(green)} green ${green === 1 ? 'one' : 'ones'}.`);
    } else {
      let streak = 0;
      for (const h of runs) {
        if (!clean(h)) break;
  ...
  function renderRibbon(history, repoUrl) {
    ...
    for (let i = runs.length; i < WINDOW; i++)
      box.append(el('span', { class: 'bar bar--empty', 'aria-hidden': 'true' }));
  ...
  function renderTiles(m, history) {
    ...
    tiles.append(
      tile(pct(t.passRate, 2), `Pass rate, run ${m.run.number}`, outcome(t), series('passRate')),
    );
    ...
    if (m.coverage) {
      ...
      tiles.append(
        tile(
          `${c.toFixed(1)} %`,
          'Line coverage, unit tests',
          c >= 75 ? 'pass' : c >= 60 ? 'flaky' : 'fail',
          series('coverageLines'),
        ),
      );
    }
    if (m.perf && m.perf.k6) {
      ...
      tiles.append(
        tile(
          ms(k.p95Ms),
          `k6 p95, ${k.profile} profile`,
          k.thresholdsOk ? 'pass' : 'fail',
          series('k6P95Ms'),
        ),
      );
    }
    if (m.lighthouse) {
      ...
      tiles.append(
        tile(
          String(l.performance),
          'Lighthouse performance',
          low >= 90 ? 'pass' : low >= 75 ? 'flaky' : 'fail',
          series('lhPerformance'),
        ),
      );
    }
    if (m.mutation) {
    ...
    if (m.security) {
      ...
      tiles.append(
        tile(
          ...
          s.error ? 'fail' : s.warning ? 'flaky' : 'pass',
          series('securityFindings'),
        ),
      );
    }
  }
  ...
  function renderTiers(m) {
    ...
    const rows = Object.entries(t.tiers || {}).map(([key, v]) => ({
      cells: [
        ...
        {
          num: true,
          node: el('span', { class: v.failed ? 'tone-fail' : null, text: n(v.failed) }),
        },
    ...
    box.append(
      table(
        [
          { text: 'Tier' },
          { text: 'Tests', num: true },
          { text: 'Passed', num: true },
          { text: 'Failed', num: true },
          { text: 'Flaky', num: true },
          { text: 'Skipped', num: true },
          { text: 'Took', num: true },
        ],
        rows,
        { label: 'Tests by tier' },
    ...
    const notes = [];
    if (typeof t.durationP95Sec === 'number')
      notes.push(
        `The slowest 5 % of tests take ${t.durationP95Sec.toFixed(1)} s or more; the mean is ${t.durationMeanSec.toFixed(1)} s.`,
      );
    if (m.kubernetes) {
      const k = m.kubernetes;
      notes.push(
        `Browser tests ran as ${word(k.shards)} Playwright shards on a kind cluster inside the runner, ${dur(k.wallSec)} wall clock${k.k6Runners ? `, with ${word(k.k6Runners)} k6 runners generating load from inside the cluster` : ''}.`,
      );
    }
  ...
  function renderK6(m) {
    ...
    box.append(
      el('p', {
        class: 'lede',
        text: `${cap(k.profile)} profile: ${n(k.requests)} requests, ${pct(k.failedRate, 2)} failed, thresholds ${k.thresholdsOk ? 'within budget' : 'crossed'}.`,
      }),
    );
    const rows = Object.entries(k.journeys || {}).map(([name, j]) => ({
      cells: [
        ...
        { num: true, text: ms(j.p99Ms) },
        {
          num: true,
          node: el('span', {
            class: j.ok ? 'tone-pass' : 'tone-fail',
            text: `${j.ok ? 'under' : 'over'} ${ms(j.budgetMs)}`,
          }),
        },
      ],
    }));
  ...
  function renderMutation(m) {
    ...
    const row = (label, value, note, tone) => ({
      cells: [
        ...
        {
          num: true,
          node: tone ? el('span', { class: `tone-${tone}`, text: value }) : undefined,
          text: tone ? undefined : value,
        },
        { text: note },
      ],
    });
    ...
    if (mu.coveredScore != null) {
      rows.push(
        row(
          'On code the tests reach',
          `${mu.coveredScore.toFixed(2)} %`,
          'how good the tests that exist are',
        ),
      );
    }
    if (mu.mutants != null)
      rows.push(row('Mutants', n(mu.mutants), 'deliberate defects introduced'));
    if (mu.survived != null) rows.push(row('Survived', n(mu.survived), 'ran, and nothing failed'));
    if (mu.classesWithNoUnitTest) {
      rows.push(
        row(
          'Classes with no unit test',
          n(mu.classesWithNoUnitTest),
          'a coverage gap, not a weak test',
        ),
      );
    }
    if (mu.floor != null)
      rows.push(row('Floor', `${mu.floor} %`, 'a ratchet; raised, never lowered'));
    box.append(
      table([{ text: 'Measure' }, { text: 'Value', num: true }, { text: 'What it means' }], rows, {
  ...
  function renderSecurity(m) {
    ...
    const rows = [
      {
        cells: [
          ...
          {
            num: true,
            node: el('span', { class: s.error ? 'tone-fail' : 'tone-pass', text: n(s.error) }),
          },
          { text: 'the ones that matter' },
        ],
      },
      {
        cells: [
          ...
          {
            num: true,
            node: el('span', { class: s.warning ? 'tone-flaky' : 'tone-pass', text: n(s.warning) }),
          },
          { text: '' },
        ],
      },
    ...
    for (const [tool, count] of Object.entries(s.tools || {})) {
      rows.push({
        cells: [
          { text: tool },
          { num: true, text: n(count) },
          { text: count ? 'findings' : 'ran, found nothing' },
        ],
      });
    }
    box.append(
      table([{ text: 'Source' }, { text: 'Count', num: true }, { text: '' }], rows, {
  ...
  function renderFlaky(m) {
    ...
    const list = m.flaky || [];
    box.replaceChildren(
      el('h2', { text: `Flaky and failing tests, last ${word(FLAKY_WINDOW)} runs` }),
    );
    box.append(
      el('p', {
        class: 'lede',
        text: 'A test is listed when it flaked or failed in any of the last ten runs. It leaves the list after ten green runs in a row, never by being removed from the report.',
      }),
    );
    if (!list.length) {
      box.append(
        el('p', { class: 'note', text: 'Nothing flaked or failed in the last ten runs.' }),
      );
      return;
    }
    const allure = (run) =>
      `${m.reports && m.reports.allure ? m.reports.allure.replace(/\d+\/?$/, '') : 'allure/'}${run}/`;
    const rows = list.map((f) => {
      const strip = Array.isArray(f.history)
        ? el(
            ...
            {
              class: 'strip',
              'aria-label': f.history
                .map((s, i) => `run ${f.lastSeen - (f.history.length - 1 - i)} ${s}`)
                .join(', '),
            },
            f.history.map((s, i) =>
              el('i', { class: s, title: `run ${f.lastSeen - (f.history.length - 1 - i)}: ${s}` }),
            ),
          )
        ...
        cells: [
          {
            node: el(
              ...
              el('span', { class: 'file', text: f.file }),
            ),
          },
          { node: strip },
          {
            num: true,
            node: el('span', {
              class: f.runsFlaky ? 'tone-flaky' : null,
              text: n(f.runsFlaky || 0),
            }),
          },
          {
            num: true,
            node: el('span', {
              class: f.runsFailed ? 'tone-fail' : null,
              text: n(f.runsFailed || 0),
            }),
          },
    ...
    box.append(
      table(
        [
          ...
          { text: 'Flaky', num: true },
          { text: 'Failed', num: true },
          { text: 'Last seen', num: true },
        ],
        rows,
        { label: 'Flaky and failing tests' },
      ),
    );
  }
  ...
  function renderReports(m, repoUrl) {
    ...
    if (r.allure)
      items.push([
        'Allure report',
        'Every tier, with the history of each test across runs',
        r.allure,
      ]);
    if (r.playwright)
      items.push([
        'Playwright report',
        'The merged browser tiers, traces on failure',
        r.playwright,
      ]);
    ...
    items.push([
      ...
      `${BASE}/quality-metrics.json`,
    ]);
    items.push([
      'metrics/history.jsonl',
      'One line per run, never pruned',
      `${BASE}/metrics/history.jsonl`,
    ]);
    box.append(
      el(
        'ul',
        ...
        items.map(([title, sub, href]) =>
          el('li', {}, el('a', { href, text: title }), el('small', { text: sub })),
        ),
      ),
    );
    if (r.allure)
      box.append(
        el(
          'p',
          { class: 'note' },
          'The newest Allure report is always at ',
          el('a', { href: r.allure.replace(/\d+\/?$/, 'latest/'), text: 'allure/latest' }),
          '.',
        ),
      );
    return repoUrl;
  ...
  function renderEmpty(err) {
    ...
    $('verdictMeta').textContent =
      'The first workflow run writes quality-metrics.json next to this page; until then there is nothing to measure.';
    $('ribbonSection').hidden = true;
    $('tiles').hidden = true;
    $('foot').textContent =
      `Could not read ${BASE}/quality-metrics.json (${err && err.message ? err.message : err}).`;
  }

  async function main() {
    try {
      const [m, history] = await Promise.all([
        fetch(`${BASE}/quality-metrics.json`, { cache: 'no-cache' }).then((r) =>
          r.ok ? r.json() : Promise.reject(new Error(`HTTP ${r.status}`)),
        ),
        fetch(`${BASE}/metrics/history.jsonl`, { cache: 'no-cache' })
          .then((r) => (r.ok ? r.text() : ''))
          .then(parseJsonl)
          .catch(() => []),
      ]);
      ...
      const mine = history.filter(
        (h) => !h.workflow || !m.run.workflow || h.workflow === m.run.workflow,
      );
