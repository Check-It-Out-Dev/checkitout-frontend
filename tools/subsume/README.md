...
| Reviewer               | `.github/workflows/ai-review.yml` (after a pipeline completes, on its artefacts; environment `ai-review`), `agent/reviewer.md`, `pr-numbers-check.mjs` (I5) | shipped | I5 spec; the first two pull requests                                                                                   |
| Proposer role          | `agent/role.md` — the box-side agent, optional: the loop itself is `propose` → `pack` → `apply` → `pr-body`                                                 | written | —                                                                                                                      |
