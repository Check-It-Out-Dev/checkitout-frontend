...
describe('shellHeadersInterceptor', () => {
  ...
  // subsumed-by: shell-headers.interceptor.spec.ts :: shellHeadersInterceptor forwards X-Email-Verification-Required to the shell-status service, shell-status.service.spec.ts :: ShellStatusService noteResponseHeaders flips consentRequired when X-Consent-Required is "1", shell-status.service.spec.ts :: ShellStatusService noteResponseHeaders does not flip on absent headers (round 1)
