...
describe('errorInterceptor', () => {
  ...
  // subsumed-by: error.interceptor.spec.ts :: errorInterceptor on 419 with no signed-in user — clears session + routes to /auth/sign-in (no refresh) (round 1)
  subsumed(it)(
    'on 401 with no signed-in user from a PROTECTED page — clears session + routes to /auth/sign-in (no refresh attempt)',
    fakeAsync(() => {
      const auth = new FakeAuth();
      const session = new FakeSession();
  ...
  // subsumed-by: error.interceptor.spec.ts :: errorInterceptor on authenticated 419 + refresh-401 (banned) — clears session + redirects + surfaces original 419 (round 1)
  subsumed(it)(
    'on authenticated 401 + refresh failure — clears session + redirects + surfaces original 401',
    fakeAsync(() => {
      const auth = new FakeAuth();
